...
export default function ResponsiveHabitRow({
    ...
    return (
        ...
        >
            {/* Mobile view */}
            <div className={`md:hidden p-3 flex flex-col space-y-2 ${status === "failed" ? "bg-muted/20" : ""}`}>
                <div className="flex justify-between items-center">
                    <div className={`font-medium line-clamp-1 ${status === "failed" ? "text-muted-foreground" : ""}`}>
                        {habit.name}
                    </div>
