  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange} modal={true}>
      <PopoverTrigger asChild>
        {children || <Button variant="outline">Select an icon</Button>}
      </PopoverTrigger>
          ...
          {displayedIcons.map(({ icon }) => (
            <TooltipProvider key={icon}>
              <Tooltip>
                <TooltipTrigger
                  className={cn(
                    "p-2 rounded-md border border-slate-200 hover:bg-slate-950/10 transition dark:border-slate-800 dark:hover:bg-slate-50/10",
