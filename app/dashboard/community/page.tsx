            ...
            try {
                ...
                try {
                    ...
                    if (Array.isArray(leaderboardResult)) {
                        ...
                        console.error("Invalid leaderboard data received:", leaderboardResult);
                ...
                toast({
                    title: "Error",
                    description: err instanceof Error ? err.message : "Failed to load community data",
                    variant: "destructive",
                });
            } finally {
                setIsLoading(false);
    ...
    return (
        <div className="w-full p-3 md:p-6 space-y-4 md:space-y-6 max-w-full">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-2 md:mb-4 w-full">
                ...
                <div className="flex space-x-2 w-full sm:w-auto">
                    <Button
                        ...
                        className="text-sm"
                    >
                        {isLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCcw className="h-4 w-4 mr-1" />}
            ...
            <Tabs defaultValue="feed" className="w-full max-w-full">
                ...
                <TabsContent value="feed" className="space-y-3 md:space-y-4 w-full max-w-full">
                    {isLoading ? (
                        // Loading skeletons
                ...
                <TabsContent value="leaderboard" className="w-full max-w-full">
                    <Card className="w-full h-fit">
                        <CardHeader>
                            ...
                            <CardDescription>
                                Top community members ranked by a composite score that rewards streak consistency, content creation, engagement, and quality.
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="px-2 md:px-6">
                            {isLeaderboardLoading ? (
                                ...
                                <div className="space-y-2 w-full">
                                    <Button
                                        variant="ghost"
                                        className="w-full justify-between mb-2 text-sm font-medium"
                                        onClick={() => setShowScoringInfo(!showScoringInfo)}
                                    >
                                        <span className="flex items-center gap-1">
                                            <InfoIcon size={16} /> How are scores calculated?
                                        </span>
                                        {showScoringInfo ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                                    </Button>

                                    {showScoringInfo && (
                                        <div className="rounded-lg bg-secondary p-4 text-sm mb-4 animate-in fade-in-50 duration-200 w-full">
                                            <ul className="list-disc pl-5 space-y-1 text-xs text-muted-foreground">
                                                <li><span className="font-medium flex items-center gap-1"><Flame size={14} /> Habit consistency:</span> Streak counts (40%)</li>
                                                <li><span className="font-medium flex items-center gap-1"><PenIcon size={14} /> Content creation:</span> Posts made (20%)</li>
                                                <li><span className="font-medium flex items-center gap-1"><Heart size={14} /> Content quality:</span> Likes received (20%)</li>
                                                <li><span className="font-medium flex items-center gap-1"><MessageSquare size={14} /> Engagement:</span> Comments made (10%)</li>
                                                <li><span className="font-medium flex items-center gap-1"><Star size={14} /> Platform points:</span> Achievements & levels (10%)</li>
                                            </ul>
                                        </div>
                                    )}
